...
class BigSMILES_StoObj:
    ...
    def __init__(self,inStr,pos=0,index=list()):
        ...
        try:
            ...
            print(errorMsg(self.rawStr,1,'Error','unrecognized element encountered during parsing the left bonding descriptor'))
            raise BigSMILES_StoObjError
        
        try:
            peeledStr = self.parseRightTerminal(peeledStr)
        except BigSMILES_BondInconsistencyError:
            print(errorMsg(self.rawStr,len(self.rawStr)-self.tmpHolder,'Error','Inconsistency in the right-end bonding descriptor'))
            raise BigSMILES_StoObjError
        except:
            print(errorMsg(self.rawStr,len(self.rawStr)-1,'Error','unrecognized element encountered during parsing the right bonding descriptor'))
            raise BigSMILES_StoObjError
        ...
        try:
            ...
            print(errorMsg(inStr,openerLen+1,'Error','unrecognized element encountered during parsing of Stochastic Object'))
            raise BigSMILES_StoObjError
        ...
        for i in range(len(res.repUnit)):
            ...
            try:
                ...
                print(errorMsg(inStr,localpos,'Error','error in parsing repeat unit ['+str(len(self.repUnit)+1)+']' ))
                raise BigSMILES_StoObjError 
            else:
                # update Bond_Dict
        ...
        for i in range(len(res.endGrp)):
            ...
            try:
                ...
                print(errorMsg(inStr,localpos,'Error','error in parsing end group ['+str(len(self.endGrp)+1) +']' ))
                raise BigSMILES_StoObjError 
            else:
                # update Bond_Dict
                for key in self.repUnit[-1].Bond_Dict.keys():
        ...
        if len(res.rawStr) < len(peeledStr):
            print(errorMsg(inStr,openerLen+1+len(res.rawStr),'Error','unrecognized element encountered during parsing of Stochastic Object'))
            raise BigSMILES_StoObjError
        #print('here4')
        return None
    ...
    def __str__(self):
        ...
        if self.leftEnd != None:
            string = string + self.leftEnd._rawStr
        ...
        if self.rightEnd != None:
            string = string + self.rightEnd._rawStr 
        string = string + '}'
        return string
...
#            return self.endGrp[key-1-len(self.repUnit)]
    ...
    def parseLeftTerminal(self,peeledStr):
        ...
        if res.rawStr == '':
            ...
            # the left end bond is perceived as if it is directly attached to the previous segment
            # ...ABC{[$/1]... ==> ABC/ + ...
            self.leftEnd = BigSMILES_Bond(res,isFirst=False,Bond_Dict=self.Bond_Dict,item=None)
            # TODO!!! error handling
#            key = self.leftEnd.getCompKey()
#            self.StoObj_Bond_Dict[key] = list()
#            self.StoObj_Bond_Dict[key].append(self.leftEnd)
            peeledStr = peeledStr[len(res.rawStr):]
        #openerLen = len(res.rawStr)
    ...
    def parseRightTerminal(self,peeledStr):
        ...
        if res.rawStr == '':
            ...
            # ...[$/1]}ABC... ==> ...;/ABC} 
            try:
                self.rightEnd = BigSMILES_Bond(res,isFirst=False,Bond_Dict=self.Bond_Dict,item=None)
            except:
