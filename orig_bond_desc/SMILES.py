...
class SMILES:
    ...
    def finalCheck(self,level,base_pos,pos,prevBond,prevAtom):
        ...
        if prevBond != None: # the chain ends with an open trailing bond, append explicit hydrogen to that
            ...
            #print(errorMsg(self.rawStr,base_pos+pos,'Error','missing ATOM, expected ATOM trailing a bond'))
            #raise SmilesError
        ...
        if level == 0:
            ...
            if not not self.ringDict:
                for i in self.ringDict.keys():
                    print(errorMsg(self.rawStr,base_pos+pos,'Error','missing ring-bond, ring ID = '+str(i)+' not closed'))
                raise SmilesError
...
#                            break
#                    if not doubleBond:
#                        print(errorMsg(self.rawStr,self.G.nodes[node1]['pos'],'Error',r'\ or / should always be adjacent to double bonds.'))
#                        raise SmilesError
            ...
            for edge in self.G.edges():
                if self.G.edges[edge]['type'] != '=':
                    ...
                    for node1 in edge:
                        ...
                        if u > 1 or d > 1:
                            print(errorMsg(self.rawStr,self.G.nodes[node1]['pos'],'Error','errorneous isomeric specification; cannot have two bonds on an allenal carbon pointing in the same relative "up" or "down" direction.'))
                            raise SmilesError
                        ...
                        # case 3: only one of the two bonds are specified
                            ...
                            if len(unspecified) > 1:
                                print(errorMsg(self.rawStr,self.G.nodes[node1]['pos'],'Error','errorneous isomeric specification; an allenal carbon with valency larger than 4 is found.'))
                                raise SmilesError
                ...
                if self.G.nodes[node]['_type'] == 'organic' or self.G.nodes[node]['_type'] == 'bracket_atom':
                    if self.G.nodes[node]['chiral'] != '':
                        ...
                        if len(self.G.nodes[node]['neighList']) != 4:
                            print(errorMsg(self.rawStr,self.G.nodes[node]['pos'],'Error','errorneous chiral center specification, chiral centers should always have 4 distinct neighbors'))
                            flag = True
                        else:
                            numH = [0,0,0]
                            ...
                            if numH[0] > 1 or numH[1] > 1 or numH[2] > 1:
                                print(errorMsg(self.rawStr,self.G.nodes[node]['pos'],'Error','the ATOM cannot be chiral with chirality '+self.G.nodes[atom]['chiral']+'with more than one H/D/T attached'))
                                flag = True
    ...
    def parseOne(self,res,level,base_pos,pos,prevBond,prevAtom):
        ...
        if 'dot' in res.keys():
            ...
            #if level > 0:
            #    print(errorMsg(self.rawStr,base_pos+pos,'dot can only be on base level and cannot appear within parenthesizes'))
            #    raise SmilesError
            # clear prevAtom
            prevAtom = None
        ...
        # the case next element is a branch
            ...
            if len(res.rawStr) == 2:
                print(errorMsg(self.rawStr,base_pos+pos,'Warning','missing expressions within parenthesizes'))
                #raise Warning
            if prevAtom == None:
                print(errorMsg(self.rawStr,base_pos+pos,'Error','expected ATOM or BOND but got BRANCH, a branch should always follow an atom'))
                raise SmilesError
            if pos == 0:
                #if level == 0:
                #    print(errorMsg(self.rawStr,base_pos+pos,'Error','expected ATOM or BOND but got BRANCH, a branch should always follow an atom'))
                #    raise SmilesError
                #else:
                print(errorMsg(self.rawStr,base_pos+pos,'Warning','ignoring extra parentheses, expected ATOM or BOND but got BRANCH, a branch should always follow an atom'))
                    #raise Warning
            if prevBond != None:
                print(errorMsg(self.rawStr,base_pos+pos,'Warning','potentially misleading expression, a branch should follow immediately an ATOM and the BOND on the main chain should come directly before the corresponding ATOM.'))
        ...
        # next element is a bond
            ...
            if prevAtom == None:
                print(errorMsg(self.rawStr,base_pos+pos,'Error','expected ATOM but got BOND, a BOND should always follow an ATOM'))
                raise SmilesError
            ...
            if prevBond != None:
                if prevBond == self.addBond(res,prevAtom):
                    #print(errorMsg(self.rawStr,base_pos+pos,'Warning','ignoring repeated BOND symbol'))
                    # raise Warning
                    pass
                else:
                    print(errorMsg(self.rawStr,base_pos+pos,'Error','expected ATOM but got BOND, a BOND should always be followed by an ATOM'))
                    raise SmilesError
        ...
        # next element is a ring-bond
            ...
            if prevAtom == None:
                print(errorMsg(self.rawStr,base_pos+pos,'Error','expected ATOM but got RING-BOND, a RING-BOND should always follow an ATOM'))
                raise SmilesError                    
            ...
            if pos == 0:
                print(errorMsg(self.rawStr,base_pos+pos,'Warning','potentially ambiguous expression, a RING-BOND should directly follow an ATOM'))
                #raise SmilesError
            ...
            if prevBond != None:
                print(errorMsg(self.rawStr,base_pos+pos,'Warning','potentially misleading expression, a RING-BOND should follow immediately an ATOM and the BOND on the main chain should come directly before the corresponding ATOM.'))
            ...
            try:
                ...
                print(errorMsg(self.rawStr,base_pos+pos,'Error','bond symbol does not match for RING-BONDS with the same ID'))
                raise SmilesError
                ...
                print(errorMsg(self.rawStr,base_pos+pos,'Error','cis-trans in RING-BOND is not consistent'))
                raise SmilesError
            except SelfLoopError:
                print(errorMsg(self.rawStr,base_pos+pos,'Error','self-loops on a single ATOM is not allowed'))
                raise SmilesError
            except MultiBondError:
                print(errorMsg(self.rawStr,base_pos+pos,'Error','more than one BOND between a pair of atoms is found'))
                raise SmilesError
        ...
        # otherwise, the next element is an atom     
            ...
            try: 
                ...
                print(errorMsg(self.rawStr,base_pos+pos,'Error','the ATOM cannot be chiral with chirality '+res.chiral+'with more than one Hydrogen attached'))
                raise SmilesError
            except AtomicWeightError:
                print(errorMsg(self.rawStr,base_pos+pos,'Error','erroneous isotope specified.'))
                raise SmilesError
            prevBond = None
